...
def convolve(image, K):
	...
	# the bodersof the input image so the spatial size are not reduced
	pad = (kW - 1) // 2
	image = cv2.copyMakeBorder(image, pad, pad, pad, pad, cv2.BORDER_REPLICATE)
	output = np.zeros((iH, iW), dtype = "float")

	# loop over the input image, "sliding" the kernel across
	# each (x,y)-coordinate from left to the right and top to bottom
	for y in np.arange(pad, iH + pad):
		for x in np.arange(pad, iW + pad):
			# extract the ROI of the image by extracting the
			# *center* region of the current (x,y)-coordinates dimentions
			roi = image[y - pad:y + pad + 1, x - pad:x + pad + 1]

			# perform the actual convolution by taking the element wise multiplication
			# between the ROI and the kernel, the summung the matrix
			k = (roi * K).sum()
...
# cinstruc average blurring kernels used to smooth an image
